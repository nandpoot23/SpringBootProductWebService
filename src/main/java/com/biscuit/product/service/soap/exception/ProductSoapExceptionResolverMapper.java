...
public class ProductSoapExceptionResolverMapper extends
		AbstractEndpointExceptionResolver {

	public ProductSoapExceptionResolverMapper() {
		this.setOrder(Integer.MIN_VALUE);
	}

	private Map<Class<? extends Exception>, EndpointExceptionResolver> resolvers = newHashMap();

	public void registerResolver(Class<? extends Exception> exceptionType,
			EndpointExceptionResolver resolver) {
	...
	protected boolean resolveExceptionInternal(MessageContext messageContext,
		...
		return resolver != null
				&& resolver.resolveException(messageContext, endpoint, ex);
	}
	...
	protected EndpointExceptionResolver findResolver(Exception ex) {
		...
		if (min.isPresent()) {
			Pair<Class<? extends Exception>, Integer> pair = (Pair<Class<? extends Exception>, Integer>) min
					.get();
	...
	private int getDepth(Class<? extends Exception> mappedClass,
			Class<? extends Exception> exceptionClass, int depth) {
